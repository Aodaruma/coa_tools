...
def collapse_short_edges(bm, obj, threshold=1.0):
    ...
    bmesh.ops.remove_doubles(
        bm, verts=verts, dist=edges_len_average * threshold)
...
def smooth_verts(bm, obj):
    ...
    for i in range(50):
        ...
        bmesh.ops.smooth_vert(bm, verts=smooth_verts, factor=1.0,
...
class COATOOLS_OT_GenerateMeshFromEdgesAndVerts(bpy.types.Operator):
    ...
    def cleanup_and_fill_mesh(self, obj, bm):
            ...
            for e1 in bm.edges:
                ...
                if edge_visible and edge_not_same:
                    i = geometry.intersect_line_line_2d(
                        e1.verts[0].co.xz, e1.verts[1].co.xz, e2[0].co.xz, e2[1].co.xz)
                    if i != None:
                        return True
            return False
        ...
        # delete double verts
        edges_len_average, shortest_edge = get_average_edge_length(
            bm, context.active_object)
        verts = []
        for edge in bm.edges:
            if not edge.hide:
        ...
        for e1 in bm.edges:
            for e2 in bm.edges:
                ...
                edges_share_points = e1.verts[0].co.xz in [
                    e2.verts[0].co.xz, e2.verts[1].co.xz] or e1.verts[1].co.xz in [e2.verts[0].co.xz, e2.verts[1].co.xz]

                if e1 != e2 and not edges_share_points and edges_not_visible:
                    i = geometry.intersect_line_line_2d(
                        e1.verts[0].co.xz, e1.verts[1].co.xz, e2.verts[0].co.xz, e2.verts[1].co.xz)
        ...
        for vert in bm.verts:
            if not vert.hide and len(vert.link_edges) == 0:
                ...
                for vert2 in bm.verts:
                    ...
                    # and edge_center not in connected_edges:
        ...
        if len(vert_loops) > 0:
            for i, loop in enumerate(vert_loops):
                for vert in loop:
                    exclude_verts = [vert.link_edges[0].other_vert(
                        vert), vert] if len(vert.link_edges) > 0 else [vert]
                    distance = 1000000000000000
                    vert_a = None
                    vert_b = None
                    if len(vert.link_edges) == 1:
                        for j, loop_next in enumerate(vert_loops):
                            for vert2 in loop_next:
                                if vert2 != vert:
                                    edge1 = (
                                        vert.co - vert.link_edges[0].other_vert(vert).co).normalized()
...
class COATOOLS_OT_TriangleFillMesh(bpy.types.Operator):
...
class COATOOLS_OT_DrawContour(bpy.types.Operator):
    ...
    def project_cursor(self, event):
        ...
        # Viewport origin
        start = bpy_extras.view3d_utils.region_2d_to_origin_3d(
            region, rv3d, coord)
    ...
    def limit_cursor_by_bounds(self, context, location):
        if self.mode == "EDIT_MESH":
            ...
            location[0] = self.limit_value(
                location[0], bounds[1][0], bounds[2][0])
            location[2] = self.limit_value(
                location[2], bounds[0][2], bounds[1][2])
    ...
    def draw_verts(self, context, obj, bm, position, use_snap=False):
        ...
        intersect_prev_vert = bm.select_history[0] if len(
            bm.select_history) > 0 else None
        for i, p in enumerate(self.intersection_points):
            for edge in bm.edges:
                sub_edge = None
                c = self.get_projected_point(
                    edge, custom_pos=p, disable_edge_threshold=True)
                if (c - p).magnitude < 0.001:
                    sub_edge = edge
                    break
            if sub_edge != None:
                divider = (
                    obj.matrix_world @ sub_edge.verts[0].co - obj.matrix_world @ sub_edge.verts[1].co).magnitude
                if divider != 0:
                    percentage = (obj.matrix_world @ sub_edge.verts[0].co - c).magnitude / (
                        obj.matrix_world @ sub_edge.verts[0].co - obj.matrix_world @ sub_edge.verts[1].co).magnitude
                    new_edge, new_vert = bmesh.utils.edge_split(
                        sub_edge, sub_edge.verts[0], percentage)
    ...
    def check_verts(self, context, event):
        ...
        for vert in bm.verts:
            if vert.select:
                verts.append(vert)
        for vert in verts:
            vert.co = context.active_object.matrix_world.inverted(
            ) @ self.limit_cursor_by_bounds(context, context.active_object.matrix_world @ vert.co)
    ...
    def get_projected_point(self, edge, custom_pos=None, disable_edge_threshold=False):
        ...
        dist = min((v1-v2).magnitude * .5, context.scene.coa_tools.snap_distance *
    ...
    def get_edge_slide_points(self, context, bm):
        ...
        snap_distance = context.scene.coa_tools.snap_distance * \
            context.space_data.region_3d.view_distance
    ...
    def snap_to_edge_or_vert(self, coord, get_bm_obj=False):
        ...
        distance = scene.coa_tools.snap_distance * \
        ...
        if self.selected_vert_coord != None and coord != None:
            ...
            for edge in bm.edges:
                if not edge.hide:  # and (edge.is_wire or edge.is_boundary):
                    e2 = [(obj.matrix_world @ edge.verts[0].co).xz,
                          (obj.matrix_world @ edge.verts[1].co).xz]
                    ip = geometry.intersect_line_line_2d(
                        e1[0], e1[1], e2[0], e2[1])
                    if ip != None:
                        ip = Vector(
                            (ip[0], self.selected_vert_coord[1], ip[1]))
                        if (ip - self.selected_vert_coord).magnitude > 0.001 and (ip - coord).magnitude > 0.001:
                            intersection_points.append(ip)
        intersection_points.sort(key=lambda x: (
            self.selected_vert_coord - x).magnitude)
    ...
    def modal(self, context, event):
        ...
        try:
            ...
            if self.in_view_3d and context.active_object != None and self.type not in ["MIDDLEMOUSE"] and self.sprite_object.coa_tools.edit_mesh and click_button not in [select_button]:
                ...
                if scene.coa_tools.surface_snap:
                    self.edge_slide_points = self.get_edge_slide_points(
                        context, bm)

                    self.snapped_vert_coord, self.point_type, self.bm_objs, self.verts_edges_data = self.snap_to_edge_or_vert(
                        self.mouse_pos_3d)
                else:
                    self.snapped_vert_coord, self.point_type, self.bm_objs, self.verts_edges_data = [
                        self.mouse_pos_3d, None, None, None]

                if self.contour_length > 0 and not self.alt:
                    self.intersection_points = self.get_intersecting_lines(
                        self.mouse_pos_3d, bm)
                else:
                    self.intersection_points = []
                ...
                if (event.value == 'PRESS' or event.value == 'CLICK') and event.type == click_button and self.mouse_press == False and not self.ctrl and not self.shift:
                    if not self.alt:
                        ...
                        self.draw_verts(context, obj, bm,
                                        self.cursor_pos_hist, use_snap=True)
                    return{'RUNNING_MODAL'}

                if (event.value == 'RELEASE' and event.type == 'MOUSEMOVE'):
                    self.mouse_press = False

                self.cur_distance = (self.mouse_pos_3d -
                                     self.cursor_pos_hist).magnitude
                self.draw_dir = (self.mouse_pos_3d -
                                 self.cursor_pos_hist).normalized()
                ...
                if not self.ctrl:
                    if self.mouse_press and self.inside_area:
                        if self.cur_distance > context.scene.coa_tools.distance:
                            i = int(self.cur_distance /
                                    (context.scene.coa_tools.distance))
                            for j in range(i):
                                new_vertex_pos = (
                                    self.cursor_pos_hist + (self.draw_dir * context.scene.coa_tools.distance))
                                self.draw_verts(
                                    context, obj, bm, new_vertex_pos, use_snap=False)
                ...
                # or self.type_prev in ["G"]:
                if (self.selected_verts_count == 0 and self.selected_vert_coord != None) or self.type in [select_button] or (self.ctrl and self.type in ["Z"]):
                    self.selected_vert_coord = None
                    self.contour_length = 0
            ...
            traceback.print_exc()
            self.report(
                {"ERROR"}, "An Error occured, please check console for more Information.")
    ...
    def exit_edit_mode(self, context, event, error=False):
        ...
        context.view_layer.objects.active = bpy.data.objects[
            self.edit_object_name] if self.edit_object_name in bpy.data.objects else None
    ...
    def finish_edit_object(self, context):
        ...
        self.texture_preview_object = bpy.data.objects[
            self.texture_preview_object_name] if self.texture_preview_object_name in bpy.data.objects else None
    ...
    def execute(self, context):
        try:
            bpy.utils.register_tool(COATOOLS_TO_DrawPolygon, after={
                                    "builtin.select"}, separator=True, group=True)
        except:
            pass
        ...
        if context.active_object == None or (context.active_object.type != "MESH" and self.mode != "DRAW_BONE_SHAPE"):
            self.report(
                {"ERROR"}, "Sprite is hidden or not selected. Cannot go in Edit Mode.")
            return{"CANCELLED"}

        if self.mode == "EDIT_MESH":
            self.edit_object, self.texture_preview_object = self.prepare_edit_object(
                context)
            self.edit_object_name = str(self.edit_object.name)
            self.texture_preview_object_name = str(
                self.texture_preview_object.name)
        ...
        if self.texture_preview_object != None and self.texture_preview_object.type == "MESH":
            ...
            self.mesh_center, self.bounds = functions.get_bounds_and_center(
                self.texture_preview_object)
...
#            bone_shape.scale = Vector((scale,scale,scale))
    ...
    def draw_callback_text(self):
        ...
        if self.mode == "EDIT_MESH":
            draw_edit_mode(self, bpy.context, color=[
                           1.0, 0.39, 0.41, 1.0], text="Edit Mesh Mode", offset=-20)
        elif self.mode == "DRAW_BONE_SHAPE":
            draw_edit_mode(self, bpy.context, color=[
                           1.0, 0.39, 0.41, 1.0], text="Draw Bone Shape", offset=-20)

    # draw_types -> LINE_STRIP, LINES, POINTS
    def draw_coords(self, coords=[], color=(1.0, 1.0, 1.0, 1.0), draw_type="LINE_STRIP", shader_type="2D_UNIFORM_COLOR", line_width=2, point_size=None):
    ...
    def coord_3d_to_2d(self, coord):
        region = bpy.context.region
        rv3d = bpy.context.space_data.region_3d
        coord_2d = bpy_extras.view3d_utils.location_3d_to_region_2d(
            region, rv3d, coord)
        return coord_2d
    ...
    def draw_callback_px(self):
        ...
        if obj != None and obj.mode == "EDIT":
            ...
            if len(self.bounds) > 3:
                vec1 = self.coord_3d_to_2d(
                    Vector(self.bounds[0]) + y_offset) + Vector((-1, -1))
                vec2 = self.coord_3d_to_2d(
                    Vector(self.bounds[1]) + y_offset) + Vector((-1, 1))
                vec3 = self.coord_3d_to_2d(
                    Vector(self.bounds[3]) + y_offset) + Vector((1, 1))
                vec4 = self.coord_3d_to_2d(
                    Vector(self.bounds[2]) + y_offset) + Vector((1, -1))
                vecs = [vec1, vec2, vec3, vec4, vec1]
                self.draw_coords(coords=vecs, color=[
                                 1, .7, .5, 1.0], draw_type=CONSTANTS.DRAW_LINE_STRIP, line_width=2)
            ...
            if functions.get_active_tool("EDIT_MESH") == "coa_tools.draw_polygon":
                ...
                if self.type not in ["K", "C", "B", "R", "G", "S"] and self.in_view_3d:
                    ...
                    if self.point_type == "VERT":
                        ...
                        p1 = self.coord_3d_to_2d(
                            obj.matrix_world @ self.verts_edges_data[0] + y_offset)
                    ...
                    # draw point
                    self.draw_coords(coords=[self.coord_3d_to_2d(
                        vertex_vec_new)], color=color, draw_type=CONSTANTS.DRAW_POINTS, point_size=8)
                    # draw intersecting edge points
                    color = yellow
                    points = []
                    for point in self.intersection_points:
                        points.append(self.coord_3d_to_2d(point))
                    self.draw_coords(coords=points, color=[
                                     0, 0, 0, 1], draw_type=CONSTANTS.DRAW_POINTS, point_size=8)
                    self.draw_coords(coords=points, color=[
                                     1, 0, .5, 1], draw_type=CONSTANTS.DRAW_POINTS, point_size=6)
                ...
                for vert in bm.verts:
                    if not vert.hide:
                        if not vert.select:
                            verts_selected.append(
                                self.coord_3d_to_2d(obj.matrix_world @ vert.co))
                        else:
                            verts.append(self.coord_3d_to_2d(
                    ...
                    if not vert.hide:
                        if len(vert.link_edges) == 0:
                            ...
                            if vert.select:
                                if self.contour_length == 0:
                                    verts_loose_selected.append(
                                        self.coord_3d_to_2d(obj.matrix_world @ vert.co))
                            else:
                                verts_loose.append(self.coord_3d_to_2d(
                                    obj.matrix_world @ vert.co))
                                self.draw_coords(coords=verts_loose, color=[
                                                 1, 1, 1, 1], draw_type=CONSTANTS.DRAW_POINTS, point_size=8)

                self.draw_coords(coords=verts_selected, color=[
                                 0, 0, 0, 1], draw_type=CONSTANTS.DRAW_POINTS, point_size=4)
                self.draw_coords(coords=verts_selected, color=[
                                 1, 0, .5, 1], draw_type=CONSTANTS.DRAW_POINTS, point_size=2)

                self.draw_coords(coords=verts, color=[
                                 0, 0, 0, 1], draw_type=CONSTANTS.DRAW_POINTS, point_size=6)
                self.draw_coords(coords=verts, color=[
                                 0, 1, .5, 1], draw_type=CONSTANTS.DRAW_POINTS, point_size=4)

                self.draw_coords(coords=verts_loose, color=[
                                 1, 0, 0, 1], draw_type=CONSTANTS.DRAW_POINTS, point_size=8)
                self.draw_coords(coords=verts_loose_selected, color=[
                                 1, .8, .8, 1], draw_type=CONSTANTS.DRAW_POINTS, point_size=8)
    ...
    def execute(self, context):
        ...
        for edge in bm.edges:
            if edge.select:
                ...
                scene.coa_tools.distance = (
                    (edge.verts[0].co - edge.verts[1].co)).magnitude/mult  # edge.calc_length()/mult
        bmesh.update_edit_mesh(obj.data)
        return {"FINISHED"}
