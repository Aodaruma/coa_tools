...
class COATOOLS_PT_Info(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        if context.scene.coa_tools.deprecated_data_found:
            row = layout.row()
            row.operator("coa_tools.convert_deprecated_data",
        ...
        if (context.space_data.shading.type != "RENDERED" or context.scene.view_settings.view_transform != "Standard") and not context.scene.coa_tools.deprecated_data_found:
            row = layout.row()
            row.operator("coa_tools.change_shading_mode",
                         text="Set Textured Shading Mode", icon="ERROR")
...
class COATOOLS_PT_ObjectProperties(bpy.types.Panel):
    ...
    def draw_outliner(self, context, layout, sprite_object, scene):
        ...
        row.prop(context.scene.coa_tools, "outliner_filter_names",
                 text="", icon="VIEWZOOM")
        if context.scene.coa_tools.outliner_favorites:
            row.prop(context.scene.coa_tools,
                     "outliner_favorites", text="", icon="SOLO_ON")
        else:
            row.prop(context.scene.coa_tools, "outliner_favorites",
                     text="", icon="SOLO_OFF")
        ...
        if sprite_object != None:
            if sprite_object.coa_tools.edit_mesh:
                row.prop(sprite_object.coa_tools, "edit_mesh",
                         text="", toggle=True, icon="LOOP_BACK")
            if sprite_object.coa_tools.edit_armature:
                row.prop(sprite_object.coa_tools, "edit_armature",
                         text="", toggle=True, icon="LOOP_BACK")
            if sprite_object.coa_tools.edit_weights:
                row.prop(sprite_object.coa_tools, "edit_weights",
                         text="", toggle=True, icon="LOOP_BACK")
            if sprite_object.coa_tools.edit_shapekey:
                row.prop(sprite_object.coa_tools, "edit_shapekey",
                         text="", toggle=True, icon="LOOP_BACK")
        ###
    ...
    def draw(self, context):
        ...
        if sprite_object != None and obj != None:
            ...
            if obj.type == "MESH" and obj.coa_tools.type == "SLOT":
                index = min(len(obj.coa_tools.slot) -
                            1, obj.coa_tools.slot_index)
                col.prop(obj.coa_tools.slot[index].mesh,
                         "name", text="", icon="OUTLINER_DATA_MESH")
            if obj.type == "ARMATURE":
                row = layout.row(align=True)
                if context.active_bone != None:

                    col.prop(context.active_bone, 'name',
                             text="", icon="BONE_DATA")
                            ...
                            for const in bone.constraints:
                                if const.type == "IK":
                                    if const.subtarget == pose_bone.name:
                                        row = col.row()
                                        row.operator(
                                            "coa_tools.remove_ik", text="Remove Bone IK", icon="CONSTRAINT_BONE")
                            ...
                            if "coa_stretch_ik_data" in bone:
                                if eval(bone["coa_stretch_ik_data"])[0] == eval(context.active_pose_bone["coa_stretch_ik_data"])[0]:
                                    if "c_bone_ctrl" == eval(bone["coa_stretch_ik_data"])[1]:
                                        row = col.row()
                                        row.label(
                                            text="Stretch IK Constraint", icon="CONSTRAINT_BONE")
                                        op = row.operator(
                                            "coa_tools.remove_stretch_ik", icon="X", emboss=False, text="")
                                        op.stretch_ik_name = eval(
                                            bone["coa_stretch_ik_data"])[0]
                                    elif eval(bone["coa_stretch_ik_data"])[1] in ["ik_bone_ctrl", "p_bone_ctrl"]:
                                        col.prop(bone, "ik_stretch",
                                                 text=bone.name)
            ...
            if obj != None and obj.type == "MESH" and "coa_base_sprite" in obj.modifiers:
                row = layout.row(align=True)
                row.prop(obj.data.coa_tools, 'hide_base_sprite',
                         text="Hide Base Sprite")
            ...
            if obj != None and obj.type == "MESH" and obj.mode == "OBJECT":
                ...
                if obj.coa_tools.type == "SLOT" and len(obj.coa_tools.slot) > 0:
                    row = layout.row(align=True)
                    slot_text = "Slot Index (" + \
                        str(len(obj.coa_tools.slot)) + ")"
                    row.prop(obj.coa_tools, 'slot_index', text="Slot Index")
                    op = row.operator(
                        "coa_tools.select_frame_thumb", text="", icon="IMAGE_RGB")
                    op = row.operator("coa_tools.add_keyframe",
                                      text="", icon="KEYTYPE_MOVING_HOLD_VEC")
                    op.prop_name = "coa_tools.slot_index"
                    op.add_keyframe = True
                    op.default_interpolation = "CONSTANT"
                    op = row.operator("coa_tools.add_keyframe",
                                      text="", icon="HANDLETYPE_ALIGNED_VEC")
                    op.prop_name = "coa_tools.slot_index"
    ...
    def draw(self, context):
        ...
        if obj != None and sprite_object != None:
            ...
            if sprite_object.coa_tools.edit_mesh == False and sprite_object.coa_tools.edit_shapekey == False and sprite_object.coa_tools.edit_armature == False and sprite_object.coa_tools.edit_weights == False and obj.mode not in ["SCULPT"]:
                row = layout.row(align=True)
                op = row.operator("coa_tools.edit_mesh",
                                  text="Edit Mesh", icon="GREASEPENCIL")
                op.mode = "EDIT_MESH"
            ...
            if sprite_object.coa_tools.edit_mesh == False and sprite_object.coa_tools.edit_shapekey == False and sprite_object.coa_tools.edit_armature == False and sprite_object.coa_tools.edit_weights == False and not (
                ...
                row = layout.row(align=True)
                row.operator("coa_tools.quick_armature",
                             text="Edit Armature", icon="ARMATURE_DATA")
            elif sprite_object.coa_tools.edit_armature:
                row = layout.row(align=True)
                row.prop(sprite_object.coa_tools, "edit_armature",
                         text="Finish Edit Armature", icon="ARMATURE_DATA")
            ...
            if sprite_object.coa_tools.edit_mesh == False and sprite_object.coa_tools.edit_shapekey == False and sprite_object.coa_tools.edit_armature == False and sprite_object.coa_tools.edit_weights == False and obj.mode not in [
                    "SCULPT"]:
                row = layout.row(align=True)
                row.operator("coa_tools.edit_shapekey",
                             text="Edit Shapekey", icon="SHAPEKEY_DATA")

            if sprite_object.coa_tools.edit_mesh == False and sprite_object.coa_tools.edit_shapekey == False and sprite_object.coa_tools.edit_armature == False and sprite_object.coa_tools.edit_weights == False and not (
                    obj.type == "MESH" and obj.mode in ["EDIT", "SCULPT"]) and (sprite_object) != None:
                row = layout.row(align=True)
                row.operator("coa_tools.edit_weights",
                             text="Edit Weights", icon="MOD_VERTEX_WEIGHT")
        ...
        if obj == None or (obj != None):
            if sprite_object != None and sprite_object.coa_tools.edit_mesh:
                row = layout.row(align=True)
                row.prop(sprite_object.coa_tools, "edit_mesh",
                         text="Finish Edit Mesh", toggle=True, icon="GREASEPENCIL")
            ...
            if obj != None and obj.mode in ["OBJECT", "POSE"] and no_edit_mode_active:
                ...
                row = layout.row(align=True)
                op = row.operator("coa_tools.create_ortho_cam",
                                  text="Create Camera", icon="OUTLINER_DATA_CAMERA")
                op.create = True
        if obj != None and obj.type == "CAMERA":
            row = layout.row(align=True)
            op = row.operator("coa_tools.create_ortho_cam",
                              text="Reset Camera Resolution", icon="OUTLINER_DATA_CAMERA")
            op.create = False

        if obj == None or (obj != None and obj.mode not in ["EDIT", "WEIGHT_PAINT", "SCULPT"]):
            row = layout.row(align=True)
            row.operator("coa_tools.create_sprite_object",
                         text="Create new Sprite Object", icon="TEXTURE_DATA")
        if obj != None and obj.type == "ARMATURE" and "sprite_object" not in obj.coa_tools:
            row = layout.row(align=True)
            row.operator("coa_tools.define_sprite_object",
                         text="Define as Sprite Object", icon="TEXTURE_DATA")
        ...
        if sprite_object != None:
            if obj != None and obj.mode in ["OBJECT", "SCULPT", "POSE"]:
                if functions.operator_exists("object.create_driver_constraint") and len(context.selected_objects) > 1:
                    row = layout.row()
                    row.operator("object.create_driver_constraint",
                                 text="Driver Constraint", icon="DRIVER")
            ...
            if sprite_object.coa_tools.edit_weights == False and sprite_object.coa_tools.edit_shapekey == False:
                if sprite_object != None and (
                    ...
                    row = layout.row(align=True)
                    row.operator("coa_tools.import_sprites",
                                 text="Re / Import Sprites", icon="FILEBROWSER")

                    row = layout.row(align=True)
                    row.operator("coa_tools.create_slot_object",
                                 text="Merge to Slot Object", icon="FILEBROWSER", emboss=True)
                    if obj != None and obj.coa_tools.type == "SLOT":
                        row.operator(
                            "coa_tools.extract_slots", text="Extract Slots", icon="EXPORT", emboss=True)

                    row = layout.row()
                    row.operator("coa_tools.change_alpha_mode",
                                 text="Change Alpha Mode", icon="SHADING_RENDERED")
                    row = layout.row()
                    row.operator("coa_tools.change_texture_interpolation_mode",
                                 text="Change Texture Mode", icon="NODE_TEXTURE")
                ...
                if obj != None:
                    if obj.type == "ARMATURE" and obj.mode == "POSE":
                        row = layout.row(align=True)
                        row.operator("coa_tools.draw_bone_shape",
                                     text="Draw Bone Shape", icon="BONE_DATA")
                    ...
                    if obj != None and obj.mode == "POSE":
                        ...
                        row = layout.row(align=True)
                        row.operator(
                            "coa_tools.set_ik", text="Create IK Bone", icon="CONSTRAINT_BONE")
                        row = layout.row(align=True)
                        row.operator("coa_tools.set_stretch_bone",
                                     text="Create Stretch Bone", icon="CONSTRAINT_BONE")

                        row = layout.row(align=True)
                        row.operator("coa_tools.create_stretch_ik",
                                     text="Create Stretch IK", icon="CONSTRAINT_BONE")
            ...
            if obj != None and obj.type == "MESH":
                ...
                if obj != None and obj.mode == "SCULPT":
                    if not sprite_object.coa_tools.edit_shapekey:
                        row = layout.row(align=True)
                        row.operator("coa_tools.leave_sculptmode",
                                     text="Finish Edit Shapekey", icon="SHAPEKEY_DATA")
                    row = layout.row(align=True)
                ...
                if sprite_object.coa_tools.edit_mesh == False and sprite_object.coa_tools.edit_shapekey == False and sprite_object.coa_tools.edit_armature == False and sprite_object.coa_tools.edit_weights == False and not (
                    ...
                    row.prop(tool_settings.unified_paint_settings,
                             "use_unified_size", text="")
                    row = col.row(align=True)
                    row.prop(tool_settings.unified_paint_settings, "strength")
                    row.prop(tool_settings.unified_paint_settings,
                             "use_unified_strength", text="")
                    row = col.row(align=True)
        ...
        if obj != None and obj.mode == "EDIT" and obj.type == "MESH" and sprite_object.coa_tools.edit_mesh:
            ...
            row = layout.row(align=True)
            operator = row.operator("coa_tools.generate_mesh_from_edges_and_verts",
                                    text="Generate Mesh", icon="OUTLINER_OB_SURFACE")
            row = layout.row(align=True)
            operator = row.operator(
                "coa_tools.fill_edge_loop", text="Triangle Fill Mesh", icon="OUTLINER_OB_SURFACE")

            col = layout.column(align=True)

            row2 = col.row(align=True)
            row2.prop(scene.coa_tools, 'distance', text="Stroke Distance")
            row2.operator("coa_tools.pick_edge_length",
            ...
            if scene.coa_tools.surface_snap:
                row2.prop(scene.coa_tools, 'surface_snap',
                          text="Snap Vertices", icon="SNAP_ON")
            else:
                row2.prop(scene.coa_tools, 'surface_snap',
                          text="Snap Vertices", icon="SNAP_OFF")

            col = layout.column(align=True)
            operator = col.operator("mesh.knife_tool", text="Knife")
            if "sprite" in obj.coa_tools:
                operator = col.operator(
                    "coa_tools.reproject_sprite_texture", text="Reproject Sprite")
...
class COATOOLS_UL_AnimationCollections(bpy.types.UIList):
    def draw_item(self, context, layout, data, item, icon, active_data, active_propname, index):
        ...
        if context.scene.coa_tools.nla_mode == "NLA" and item.name not in ["NO ACTION", "Restpose"]:
            ...
            op = col.operator("coa_operator.create_nla_track",
                              icon="NLA", text="")
...
class COATOOLS_UL_EventCollection(bpy.types.UIList):
    def draw_item(self, context, layout, data, item, icon, active_data, active_propname, index):
        ...
        if item.collapsed:
            row.prop(item, "collapsed", emboss=False,
                     text="", icon="TRIA_RIGHT")
        else:
            row.prop(item, "collapsed", emboss=False,
                     text="", icon="TRIA_DOWN")
        row.prop(item, "frame", emboss=True, text="Frame")
        op = row.operator("coa_tools.remove_timeline_event",
        ...
        if not item.collapsed:
            row = col.row(align=True)
            # row.alignment = "RIGHT"
            op = row.operator("coa_tools.add_event", icon="ADD",
                              text="Add new Event", emboss=True)
            op.index = index
            for i, event in enumerate(item.event):
                row = col.row(align=True)
                row.prop(event, "type", text="")
                row.prop(event, "value", text="")
                op = row.operator("coa_tools.remove_event",
                                  icon="PANEL_CLOSE", text="", emboss=True)
                op.index = index
                op.event_index = i
...
class COATOOLS_OT_SelectChild(bpy.types.Operator):
    ...
    mode: EnumProperty(
        items=(("object", "object", "object"), ("bone", "bone", "bone")))
    armature = None
...
class COATOOLS_PT_Collections(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        if sprite_object != None:
            ...
            box_col.template_list("COATOOLS_UL_AnimationCollections", "dummy", sprite_object.coa_tools, "anim_collections",
                                  sprite_object.coa_tools, "anim_collections_index", rows=2, maxrows=10, type='DEFAULT')
            col = row.column(align=True)
            col.operator("coa_tools.add_animation_collection",
                         text="", icon="ADD")
            col.operator("coa_tools.remove_animation_collection",
                         text="", icon="REMOVE")

            if len(sprite_object.coa_tools.anim_collections) > 2 and sprite_object.coa_tools.anim_collections_index > 1:
                col.operator(
                    "coa_tools.duplicate_animation_collection", text="", icon="COPY_ID")

            if not "-nonnormal" in context.screen.name:
                col.operator("coa_tools.toggle_animation_area",
                             text="", icon="ACTION")
            ...
            if len(sprite_object.coa_tools.anim_collections) > 0 and sprite_object.coa_tools.anim_collections[sprite_object.coa_tools.anim_collections_index].action_collection:
                row = layout.row(align=True)
                item = sprite_object.coa_tools.anim_collections[
                    sprite_object.coa_tools.anim_collections_index]
                ...
                row.template_list("COATOOLS_UL_EventCollection", "dummy", item,
                                  "timeline_events", item, "event_index", rows=1, maxrows=10, type='DEFAULT')
                col = row.column(align=True)
                col.operator("coa_tools.add_timeline_event",
                             text="", icon="ADD")
            ...
            if context.scene.coa_tools.nla_mode == "ACTION":
                ...
                operator = row.operator(
                    "coa_tools.batch_render", text="Batch Render Animations", icon="RENDER_ANIMATION")
            else:
                operator = row.operator(
