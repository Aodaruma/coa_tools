...
def set_uv_image(obj):
    ...
    if len(data.materials) > 0:
        ...
        tex = [slot.texture for slot in mat.texture_slots if slot !=
               None and slot.texture.type == "IMAGE"][0]
...
def draw_sculpt_ui(self, context, layout):
    ...
    if obj != None and obj.mode == "SCULPT":
        ...
        op = subrow.operator("coa_tools.shapekey_add",
                             icon="FILE_NEW", text="")

        if obj.data.shape_keys != None and len(obj.data.shape_keys.key_blocks) > 0:
            op = subrow.operator("coa_tools.shapekey_rename",
                                 icon="OUTLINER_DATA_FONT", text="")
            op = subrow.operator(
                "coa_tools.shapekey_remove", icon="X", text="")
        ...
        if obj.data.shape_keys != None:
            ...
            if shapekey_index > 0:
                ...
                subrow.prop(obj, "show_only_shape_key",
                            text="", icon="UNPINNED")
        ...
        if not context.particle_edit_object:
            ...
            col.template_ID_preview(
                settings, "brush", new="brush.add", rows=3, cols=8)
        ...
        subrow = col.row(align=True)
        subrow.prop(settings, "use_locked_size",
                    text="", toggle=True, icon=icon)
        ...
        row.operator("brush.curve_preset", icon='SHARPCURVE',
                     text="").shape = 'SHARP'
        row.operator("brush.curve_preset", icon='LINCURVE',
                     text="").shape = 'LINE'
        row.operator("brush.curve_preset", icon='NOCURVE',
                     text="").shape = 'MAX'
...
def check_region(context, event):
    ...
    if context.area != None:
        if context.area.type == "VIEW_3D":
            ...
            view_3d_region_x = Vector(
                (context.area.x + t_panel.width, context.area.x + context.area.width - n_panel.width))
            view_3d_region_y = Vector(
                (context.region.y, context.region.y+context.region.height))
...
def unwrap_with_bounds(obj, uv_idx):
    ...
    offset = [get_local_dimension(
        obj)[2][0] * scale_x, get_local_dimension(obj)[2][1] * scale_z]
...
def set_action(context, item=None):
    sprite_object = get_sprite_object(context.active_object)
    if item == None:
        index = min(len(sprite_object.coa_tools.anim_collections) -
                    1, sprite_object.coa_tools.anim_collections_index)
...
def actions_callback(self, context):
    actions = []
    actions.append(("New Action...", "New Action...",
                    "New Action", "FILE_NEW", 0))
    actions.append(("Clear Action", "Clear Action",
                    "Clear Action", "RESTRICT_SELECT_ON", 1))
    i = 2
                    ...
                    if active_space_data != None:
                        if hasattr(active_space_data, "region_3d"):
                            ...
                            bpy.ops.view3d.view_axis(
                                override, type='FRONT', align_active=False, relative=False)
...
def set_uv_default_coords(context, obj):
    ...
    pos_y = frame_size.y * - \
        int(int(obj.coa_sprite_frame) / int(obj.coa_tiles_x))
    ...
    if "coa_sprite" in obj and obj.mode == "OBJECT":
        ...
        for i, coord in enumerate(obj.data.uv_layers[obj.data.uv_layers.active.name].data):
            if i < len(obj.coa_uv_default_state):
                coord.uv = Vector((obj.coa_uv_default_state[i].uv[0] / obj.coa_tiles_x,
                                   obj.coa_uv_default_state[i].uv[1] / obj.coa_tiles_y)) + frame + offset
...
def update_verts(context, obj):
    ...
    if "coa_sprite" in obj:
        ...
        for vert in obj.data.vertices:
            vert.co[0] = (vert.co[0] / obj.coa_dimensions_old[0] * sprite_sheet_width /
                          obj.coa_tiles_x * scale_x * obj.matrix_local.to_scale()[0])
            vert.co[2] = (vert.co[2] / obj.coa_dimensions_old[2] * sprite_sheet_height /
                          obj.coa_tiles_y * scale_y * obj.matrix_local.to_scale()[2])
...
def change_slot_mesh_data(context, obj, obj_eval=None):
    if len(obj.coa_tools.slot) > 0:
        ...
        obj.coa_tools["slot_index"] = min(
            obj.coa_tools.slot_index, max(0, len(obj.coa_tools.slot)-1))
        if obj_eval == None:
            obj_eval = obj
        idx = max(min(obj_eval.coa_tools.slot_index,
                      len(obj.coa_tools.slot)-1), 0)
...
def display_children(self, context, obj):
    ...
    if obj != None:
        ...
        if context.scene.coa_tools.display_all:
            row.prop(context.scene.coa_tools, "display_all", text="",
                     toggle=True, icon="DISCLOSURE_TRI_RIGHT")
        else:
            row.prop(context.scene.coa_tools, "display_all", text="",
                     toggle=True, icon="DISCLOSURE_TRI_DOWN")
        ...
        if sprite_object != None:
            if sprite_object.coa_tools.favorite:
                row.prop(sprite_object.coa_tools,
                         "favorite", text="", icon="SOLO_ON")
            else:
                row.prop(sprite_object.coa_tools, "favorite",
                         text="", icon="SOLO_OFF")
        ...
        for obj2 in context.view_layer.objects:
            if get_sprite_object(obj2) == obj2:
                in_range = current_display_item in range(context.scene.coa_tools.display_page * context.scene.coa_tools.display_length,
                                                         context.scene.coa_tools.display_page * context.scene.coa_tools.display_length + context.scene.coa_tools.display_length)
                ...
                if in_range or context.scene.coa_tools.display_all:
                    ...
                    if get_sprite_object(obj2) == sprite_object:
                        if obj2.coa_tools.show_children and get_sprite_object(obj2) == sprite_object:
                            subrow.prop(obj2.coa_tools, "show_children",
                                        text="", icon="TRIA_DOWN", emboss=False)
                        else:
                            subrow.prop(obj2.coa_tools, "show_children",
                                        text="", icon="TRIA_RIGHT", emboss=False)
                    op = subrow.operator(
                        "coa_tools.select_child", text=obj2.name, emboss=False)
                    op.mode = "object"
                    ...
                    if armature.mode == "OBJECT":
                        ...
                        icon = "EDITMODE_HLT"
                    op = subrow.operator(
                        "coa_tools.toggle_pose_mode", icon=icon, text="", emboss=False)
                    op.ob_name = armature.name

                    op = subrow.operator(
                        "coa_tools.view_sprite", icon="ZOOM_SELECTED", text="", emboss=False)
                    op.type = "VIEW_ALL"
                    op.name = obj2.name
                    ...
                    if not obj2.coa_tools.change_z_ordering:
                        row2.active = False
                    row2.prop(obj2.coa_tools, "change_z_ordering",
                              text="", icon="SORTALPHA", emboss=False)

                current_display_item += 1
                if obj2 == sprite_object:
                    draw_children(self, context, sprite_object, layout, box, row,
                                  col, children, obj, current_display_item, disable_list)
...
def draw_children(self, context, sprite_object, layout, box, row, col, children, obj, current_display_item, disable_list):
    ...
    if disable_list:
        if sprite_object.coa_tools.edit_mesh:
            box.row().prop(sprite_object.coa_tools, "edit_mesh",
                           text="", toggle=True, icon="LOOP_BACK")
        if sprite_object.coa_tools.edit_armature:
            box.row().prop(sprite_object.coa_tools, "edit_armature",
                           text="", toggle=True, icon="LOOP_BACK")
        if sprite_object.coa_tools.edit_weights:
            box.row().prop(sprite_object.coa_tools, "edit_weights",
                           text="", toggle=True, icon="LOOP_BACK")
        if sprite_object.coa_tools.edit_shapekey:
            box.row().prop(sprite_object.coa_tools, "edit_shapekey",
                           text="", toggle=True, icon="LOOP_BACK")

    if sprite_object != None and sprite_object.coa_tools.show_children:
        children = sorted(children, key=lambda x: x.location[1] if type(
            x) == bpy.types.Object else x.name, reverse=False)
        children = sorted(children, key=lambda x: x.type if type(
            x) == bpy.types.Object else x.name, reverse=False)
        ...
        for i, child in enumerate(children):
            ...
            if child.name in active_collection.objects:
                ...
                name_found = obj.coa_tools.filter_names.upper(
                ) in child.name.upper() and not obj.coa_tools.filter_names == ""
                current_display_item += 1
                    ...
                    if (sprite_object.coa_tools.favorite and child.coa_tools.favorite) or not sprite_object.coa_tools.favorite or (child.type == "ARMATURE" and (favorite_bones(child))):
                        if obj.coa_tools.filter_names.upper() in child.name.upper():
                            ...
                            if child.type == "MESH" and len(child.data.vertices) > 4 and child.data.coa_tools.hide_base_sprite == False and "coa_base_sprite" in child.vertex_groups and "coa_base_sprite" in child.modifiers:
                                subrow2.prop(
                                    child.data.coa_tools, 'hide_base_sprite', text="", icon="ERROR", emboss=False)
                            else:
                                subrow2.separator()
                                subrow2.separator()
                                subrow2.separator()
                            ...
                            if child.type == "ARMATURE":
                                ...
                                op = subrow2.operator(
                                    "coa_tools.advanced_settings", text="", icon="TEXTURE", emboss=False)
                            ...
                            op = subrow2.operator(
                                "coa_tools.select_child", text=name, emboss=False)
                            op.mode = "object"
                            ...
                            if child.type == "MESH" and child.coa_tools.type == "SLOT":
                                if child.coa_tools.slot_show:
                                    subrow2.prop(
                                        child.coa_tools, "slot_show", text="", icon="TRIA_DOWN", emboss=False)
                                else:
                                    subrow2.prop(
                                        child.coa_tools, "slot_show", text="", icon="TRIA_LEFT", emboss=False)

                                if sprite_object.coa_tools.edit_armature:
                                    op = subrow2.operator(
                                        "coa_tools.bind_mesh_to_bones", text="", icon="BONE_DATA", emboss=False)
                                    op.ob_name = child.name

                            if not sprite_object.coa_tools.change_z_ordering:
                                if child.coa_tools.favorite:
                                    subrow2.prop(
                                        child.coa_tools, "favorite", emboss=False, text="", icon="SOLO_ON")
                                else:
                                    subrow2.prop(
                                        child.coa_tools, "favorite", emboss=False, text="", icon="SOLO_OFF")
                                ...
                                if child.coa_tools.hide:
                                    ...
                                    if not child.hide_viewport:
                                        icon = "HIDE_OFF"
                                    subrow2.prop(
                                        child.coa_tools, "hide", emboss=False, text="", icon=icon)
                                else:
                                    subrow2.prop(
                                        child.coa_tools, "hide", emboss=False, text="", icon="HIDE_OFF")
                                if child.coa_tools.hide_select:
                                    subrow2.prop(
                                        child.coa_tools, "hide_select", emboss=False, text="", icon="RESTRICT_SELECT_ON")
                                else:
                                    subrow2.prop(
                                        child.coa_tools, "hide_select", emboss=False, text="", icon="RESTRICT_SELECT_OFF")
                            else:
                                children_names = []
                                for child in children:
                                ...
                                if child_obj.type == "MESH":
                                    op = subrow2.operator(
                                        "coa_tools.change_z_ordering", text="", icon="TRIA_DOWN")
                                    op.index = i
                                    op.direction = "DOWN"
                                    op.active_sprite = children[i].name
                                    op.all_sprites = str(children_names)
                                    op = row.operator(
                                        "coa_tools.change_z_ordering", text="", icon="TRIA_UP")
                                    op.index = i
                                    op.direction = "UP"
                                    op.active_sprite = children[i].name
                                    op.all_sprites = str(children_names)
                            ...
                            if child.type == "MESH" and child.coa_tools.type == "SLOT" and child.coa_tools.slot_show:
                                for i, slot in enumerate(child.coa_tools.slot):
                                    ...
                                    if slot.active:
                                        subrow.prop(
                                            slot, "active", text=name, icon="RADIOBUT_ON", emboss=False)
                                    else:
                                        subrow.prop(
                                            slot, "active", text=name, icon="RADIOBUT_OFF", emboss=False)

                                    subrow = row.row(align=True)

                                    op = subrow.operator(
                                        "coa_tools.move_slot_item", icon="TRIA_DOWN", emboss=False, text="")
                                    op.idx = i
                                    op.ob_name = child.name
                                    op.mode = "DOWN"
                                    op = subrow.operator(
                                        "coa_tools.move_slot_item", icon="TRIA_UP", emboss=False, text="")
                                    op.idx = i
                                    op.ob_name = child.name
                                    op.mode = "UP"
                                    ...
                                    op = subrow.operator(
                                        "coa_tools.remove_from_slot", icon="PANEL_CLOSE", emboss=False, text="")
                                    op.idx = i
                                    op.ob_name = child.name
...
def draw_bone_entry(self, bone, row, col, child, indentation_level=0):
    ...
    if bone.coa_tools.favorite:
        row.prop(bone.coa_tools, "favorite",
                 emboss=False, text="", icon="SOLO_ON")
    else:
        row.prop(bone.coa_tools, "favorite",
                 emboss=False, text="", icon="SOLO_OFF")
    if bone.hide:
        row.prop(bone.coa_tools, "hide", text="", emboss=False, icon="HIDE_ON")
    else:
        row.prop(bone.coa_tools, "hide", text="",
                 emboss=False, icon="HIDE_OFF")
    if bone.hide_select:
        row.prop(bone.coa_tools, "hide_select", text="",
                 emboss=False, icon="RESTRICT_SELECT_ON")
    else:
        row.prop(bone.coa_tools, "hide_select", text="",
                 emboss=False, icon="RESTRICT_SELECT_OFF")
